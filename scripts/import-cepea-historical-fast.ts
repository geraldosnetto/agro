/**
 * Script OTIMIZADO para importar dados históricos do CEPEA
 * Usa batch inserts para performance (~100x mais rápido)
 * Execute com: npx tsx scripts/import-cepea-historical-fast.ts
 */
...
const FILE_TO_SLUG_MAP: Record<string, { slug: string; praca: string; estado: string }> = {
    ...
    // Etanol Mensal (NOVOS - estavam faltando)
...
// Cache de cotações existentes para evitar duplicatas
async function getExistingDates(commodityId: string, praca: string): Promise<Set<string>> {
    const existing = await prisma.cotacao.findMany({
        where: { commodityId, praca },
        select: { dataReferencia: true }
    });
    return new Set(existing.map(e => e.dataReferencia.toISOString()));
}
...
async function importFile(filePath: string): Promise<{ imported: number; skipped: number; error?: string }> {
    try {
        ...
        const existingDates = await getExistingDates(commodityId, mapping.praca);
        ...
        for (let i = startRow; i < data.length; i++) {
            ...
            if (existingDates.has(dataRef.toISOString())) {
                skipped++;
                continue;
            }
...
async function main() {
    console.log('📊 Iniciando importação OTIMIZADA de dados históricos CEPEA...\n');
